   ...
   public class clsFunction
    {
        ...
        public static DataTable ExcuteProc(string namePro, string[,] arrParameters)
        {
        ...
        public static void sysGrantUserByRole(DevExpress.XtraBars.Bar ctr, string FormName)
        {
            try
            {
                ...
                {
                    dt = ExcuteProc("getRoleUserByRole", new string[2, 2] { { "IDUser", _iduser }, { "FormName", FormName } });
                }
                else
                {
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < ctr.Manager.Items.Count; i++)
                    {
                        ...
                        option = arr.Length > 1 ? arr[1] + "_" + arr[2] : "";
                        switch (option)
                        {
                            case "allow_insert":
        ...
        public static bool Delete(Form F, DevExpress.XtraGrid.Views.Grid.GridView GvDM)
        {
            bool flag = false;
        ...
        public static bool Delete(Form F, DevExpress.XtraGrid.Views.Grid.GridView GvDM,int index)
        {
            bool flag = false;
            DataTable dtsysMessage = new DataTable();
            dtsysMessage = APCoreProcess.APCoreProcess.Read("sysMessage");
        ...
        public static bool Delete_M(string table, DevExpress.XtraGrid.Views.Grid.GridView GvDM,string colName, Form F, GridColumn gc,string btn, string tablePre, string field)
        {
            bool flag=false;
            try
            {
                if (GvDM.FocusedRowHandle >= 0)
                {
                    ...
                    if (tablePre !="" ? APCoreProcess.APCoreProcess.Read("select " + field + " from " + tablePre + " where " + field + "='" + GvDM.GetRowCellValue(GvDM.FocusedRowHandle, gc.Name).ToString() + "'").Rows.Count == 0 : 1 == 1)
                    {
                        if (MessageDelete("Thông báo", "Bạn có muốn xóa mẫu tin này không?"))
                        {
                            ...
                            ds.Tables.Add(APCoreProcess.APCoreProcess.Read("SELECT * FROM " + table + " where " + (gc.Name) + " = '" + GvDM.GetRowCellValue(GvDM.FocusedRowHandle, gc.Name).ToString() + "'"));
        ...
        public static bool Delete_M_Not_Pre(string table, DevExpress.XtraGrid.Views.Grid.GridView GvDM, string colName, Form F, GridColumn gc, string btn, string field)
        {
            bool flag = false;
            try
            {
       ...
       public static String Insert_data(Control root, string form_name, String code)
        {
            code = "";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                try
                {
                    ...
                    {
                        string field = dt.Rows[i]["control_name"].ToString();
                        string val = getDataControl(FindControl(root, dt.Rows[i]["control_name"].ToString()));
                        if (field.Contains("_IK1"))
                        {
                            code = autocode(val, getNameControls(form_name), getNameControls(field));
                            dr[getNameControls(field)] = code;
                        }
                        else
                        {
                            if (val == "")
            ...
            if (Function.clsFunction._keylience != true)
            {
                MessageBox.Show("Error","Bug lost quotation");
                Application.Exit();
            }

            if (code == "")
            {
                MessageBox.Show("Error", "Không tạo được số báo giá");
            }
            return code;
           ...
           for (int i = 0; i < dt.Rows.Count; i++)
           {
               try
               {
                   ...
                   else
                   {
                       ...
                       if (field.Contains("_IK1"))
                       {
                           String code = autocode(val, getNameControls(form_name), getNameControls(field));
                           dr[getNameControls(field)] = code;
                       }
                       else
                       {
       ...
       private static string autocode(string scode, string table, string field)
       {
           string newcode="";
           string sign = scode.Substring(0, scode.Length - 6);
           int val = Convert.ToInt32( scode.Substring(scode.Length - 6, 6));
           if (APCoreProcess.APCoreProcess.Read("select * from " + table + " where " + field + " = '" + scode + "'").Rows.Count > 0)
           {
               val = val + 1;
           }
           newcode = sign +  val.ToString().PadLeft(6,'0');
           return newcode;
       }
...
string[] column_summary, string sql_grid,string grid_name)
        {
            DataTable dt = new DataTable();
            APCoreProcess.APCoreProcess.ExcuteSQL("delete from sysGridColumns where form_name='" + F.Name + "' and grid_name='" + grid_name + "'");
            dt = APCoreProcess.APCoreProcess.Read("sysGridColumns");
            try
            {
                DataRow dr = dt.NewRow();
...
string[] column_summary, string sql_grid, string tree_name)
       ...
       public static bool checkAdmin(){
           bool flag = false;
           if (APCoreProcess.APCoreProcess.Read("select * from sysUser where userid = '" + _iduser + "' and root = 1").Rows.Count > 0)
           {
               flag = true;
           }
           return flag;
        ...
        public static int nghigiuaca(string macakip)
       {
        ...
        public static void customPopupMenu(BarManager bar_menu_C, PopupMenu menu, GridView gv, Form F)
        {
            ...
            DataTable dt = new DataTable();
            dt = APCoreProcess.APCoreProcess.Read("select  column_visible from sysGridColumns where form_name='" + F.Name + "' and grid_name='"+ gv.Name +"' ");
            if (dt.Rows.Count > 0)
            {
        ...
        public static int findIndexInArray(string value, string[] arr)
        {
            int index = -1;
        ...
        public static void customPopupMenu(BarManager bar_menu_C, Dictionary<string, string>lstMenu, PopupMenu menu, GridView gv, Form F)
        {
            // Bind the menu to a bar manager.         
            menu.Manager = bar_menu_C;
            menu.ItemLinks.Clear();
            // Add two items that belong to the bar manager.  
            int k = 0;
            bar_menu_C.Items.Clear();
            foreach (KeyValuePair<string, string> pair in lstMenu)
            {

                BarEditItem bi = new BarEditItem();
                bi.Caption = transLateText(pair.Key);
                bi.Name = transLateText(pair.Value);
                bar_menu_C.Items.Add(bi);
                menu.ItemLinks.Add(bar_menu_C.Items[k]);
                k++;
            }
            BarSubItem bit = new BarSubItem();
            bit.Caption = transLateText("Ẩn/Hiện cột");
        ...
        public static string GetEmpNameByUser()
        {
            string manv = "";
            DataTable dt = new DataTable();
            dt = APCoreProcess.APCoreProcess.Read("select E.StaffName from sysUser S INNER JOIN EMPLOYEES E ON S.idemp = E.idemp  where userid='" + Function.clsFunction._iduser + "'");
            if (dt.Rows.Count > 0)
                manv = dt.Rows[0][0].ToString();
            return manv;
        }
       ...
       // xuat ra thu
        ...
        public static string layMa(string sDauMa, string field, string table)// autocode 
        {
            ...
            DataTable danhSach = APCoreProcess.APCoreProcess.Read("select TOP 1 " + field + " from " + table + " ORDER BY " + field + " DESC ");
            string XuatMa = sDauMa;
            int ma = 0;
            string maCuoi = danhSach.Rows.Count > 0 ? danhSach.Rows[0][field].ToString() : "0";//KD11
            try
            {
        ...
        public static int autonumber(string field, string table)// autocode 
        {
            ...
            int maCuoi = Convert.ToInt32(danhSach.Rows.Count - 1 >= 0 ? danhSach.Rows[danhSach.Rows.Count - 1][0].ToString() : "0");
            num = maCuoi + 1;
            }
            else
            {
        ...
        public static string autonumber(string sDauma,string field, string table)// autocode 
        {
            ...
            int num = 0;
            int maCuoi = Convert.ToInt32(danhSach.Rows.Count - 1 >= 0 ? danhSach.Rows[danhSach.Rows.Count - 1][0].ToString().Replace(sDauma,"") : "0");
            num = maCuoi + 1;
            return sDauma+ num;
        }
            ...
            try
            {
                ...
                for (int i = 0; i < a; i++)
                {
                    for (int j = 0; j < b; j++)
                    {
                        ...
                        try
                        {
                            value = array1[i].Split(separator2.ToCharArray())[j];
                        }
                        catch { }
                        Array2[i, j] = value;
                        k++;
  ...
  ? 1 : 0;
       ...
       public static bool MessageDelete(string title, string text)
       {
        ...
        public static string transLateText(string text)
        {
            ...
            if (text !="")
            {
                ...
                if (APCoreProcess.APCoreProcess.IssqlCe == false)
                {
                    dt = clsFunction.ExcuteProc("Tranlate", new string[2, 2] { { "type", Function.clsFunction.langgues }, { "str", text } });
                }
                else
                {
                ...
                try
                {
                    if (ctl != null)
                    {

                        {
                            ...
                            if (ctl.GetType().Name.ToString().Trim().ToUpper() == "SpinEdit".ToUpper().Trim())
                            {
                                data += ((SpinEdit)ctl).Value + "/";
                            }
                            if (ctl.GetType().Name.ToString().Trim().ToUpper() == "DateEdit".ToUpper().Trim())
                            {
                                if ((((DateEdit)ctl)) != null && (((DateEdit)ctl).Text) != "")
                                {
                            ...
                            if (ctl.GetType().Name.ToString().Trim().ToUpper() == "GridLookupEdit".ToUpper())
                            {
                                try
                                {
                                    data += ( ((GridLookUpEdit)ctl).EditValue==null ? "" : ((GridLookUpEdit)ctl).EditValue.ToString()).ToString().Trim() + "/";
                                }
                                catch (Exception ex)
                                {
                                    DevExpress.XtraEditors.XtraMessageBox.Show(ex.Message, "Thông báo");
        ...
        public static int GetNumberInStringBK(string pValue)
        {
            ...
            if (number == 0)
                number =Convert.ToInt32(pValue.Substring(pValue.Length-2, 2));
            return number;
        }
        ...
        public static string getNameControls(string name)
        {
            ...
            {
                MessageBox.Show("Thông báo lỗi : ", "Error " + MethodBase.GetCurrentMethod().Name + ": " + ex.Message, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return fielname;
        ...
        public static void CreateTable(Control ctl, Form F)
        {
            ...
            for (int i = 0; i < ctl.Controls.Count; i++)
            {
                ...
                {
                    if (ctl.Controls[i].Name.Contains("_I") != false && ctl.Controls[i].Name.Contains("_S") != true)
                    {
                        try
                        {
            ...
            for (int i = 0; i < ctl.Controls.Count; i++)
            {
                ...
                else
                {
                    if (ctl.Controls[i].Name.Contains("I") != false)
                    {
                        ...
                        if (ctl.Controls[i].GetType().Name.ToString().Trim().ToUpper() == "CalcEdit".ToUpper().Trim()) 
                        {
                            ((CalcEdit)ctl.Controls[i]).Properties.MaxLength = Convert.ToInt16(getLastNumControls(ctl.Controls[i].Name));
                        }
                    }
                }      
            }      
                        ...
                        if (dt1.Rows[i]["field_name"].ToString().ToUpper().Trim() == Function.clsFunction.getNameControls(Function.clsFunction.FindControl(F, dt1.Rows[i]["control_name"].ToString()).Name.ToString().ToUpper().Trim()))
                        {
                            ...
                            try
                            {
                                ...
                                if (Function.clsFunction.FindControl(F, dt1.Rows[i]["control_name"].ToString()).GetType().Name == "RadioGroup")
                                {
                                    try
                                    {
                                        ((RadioGroup)Function.clsFunction.FindControl(F, dt1.Rows[i]["control_name"].ToString())).EditValue = Convert.ToInt32(dt.Rows[0][Function.clsFunction.getNameControls(Function.clsFunction.FindControl(F, dt1.Rows[i]["control_name"].ToString()).Name.ToString().ToUpper().Trim())].ToString());
                                    }
                                    catch 
                                    {
                                        ((RadioGroup)Function.clsFunction.FindControl(F, dt1.Rows[i]["control_name"].ToString())).EditValue = Convert.ToInt32(dt.Rows[0][Function.clsFunction.getNameControls(Function.clsFunction.FindControl(F, dt1.Rows[i]["control_name"].ToString()).Name.ToString().ToUpper().Trim())].ToString());
                                ...
                                if (Function.clsFunction.FindControl(F, dt1.Rows[i]["control_name"].ToString()).GetType().Name == "SpinEdit")
                                {
                                    ((SpinEdit)Function.clsFunction.FindControl(F, dt1.Rows[i]["control_name"].ToString())).Value = Convert.ToInt32( dt.Rows[0][Function.clsFunction.getNameControls(Function.clsFunction.FindControl(F, dt1.Rows[i]["control_name"].ToString()).Name.ToString().ToUpper().Trim())]);
                                    continue;
                                }
                            }
                            catch (Exception ex)
        ...
        public static void AddKeyDownEvent(Control obj)
        {
            try
            {
                Control ctl = obj.GetNextControl(obj, true);
                while (!(ctl == null))
                {
                    ctl.KeyDown += AllControls_KeyDown;
        ...
        private static void AllControls_KeyDown(Object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter && isNext==false)
            {                
                SendKeys.Send("{TAB}");
                e.Handled = true;              
            }
       ...
       {
           DateTime dt = (DateTime)APCoreProcess.APCoreProcess.Read("select getdate()").Rows[0][0];
           return dt;
       }

       public static bool checkIsmanager(String idemp)
       {
           DataTable dt = APCoreProcess.APCoreProcess.Read("select * from employees where idemp ='"+ idemp +"' and ismanager =1");
           if (dt.Rows.Count > 0)
               return true;
           return false;
       }

       public static bool checkIsmanagerPo(String idemp)
       {
           DataTable dt = APCoreProcess.APCoreProcess.Read("select * from employees where idemp ='" + idemp + "' and ismanagerpo =1");
           if (dt.Rows.Count > 0)
               return true;
           return false;
       }

       public static DataTable checkBaotri()
       {
           DataTable dt = new DataTable();
           try
           {
               dt = APCoreProcess.APCoreProcess.Read("select * from v_thongbao_baotri B inner join EMPLOYEES E ON B.idemp = E.IDEMP  where  idrecursive like '%" + Function.clsFunction.GetIDEMPByUser() + "%'");
           }
           catch (Exception ex)
           {
               clsFunction.MessageInfo("Thông báo", ex.Message);
