    ...
    public partial class frm_Grant_SH : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void GetValueClockUser(bool value)
        {
            ...
            if (va_lue == true && !checkAdmin(tl_containmenu_S.FocusedNode.GetValue(colID).ToString()))
            {
                string mavt = "";
                mavt = getVaiTroByUser(tl_containmenu_S.FocusedNode.GetValue(colID).ToString());
                APCoreProcess.APCoreProcess.ExcuteSQL("update  sysUser set status=0 where userid='" + tl_containmenu_S.FocusedNode.GetValue(colID).ToString() + "'");
            }
            else
            {
        ...
        public void GetValueUnClockUser(bool value)
        {
            ...
            if (va_lue == true && !checkAdmin(tl_containmenu_S.FocusedNode.GetValue(colID).ToString()))
            {
                string mavt = "";
                mavt = getVaiTroByUser(tl_containmenu_S.FocusedNode.GetValue(colID).ToString());
                APCoreProcess.APCoreProcess.ExcuteSQL("update  sysUser set status=1 where userid='" + tl_containmenu_S.FocusedNode.GetValue(colID).ToString() + "'");
            }
            else
            {
                MessageBox.Show("Bạn không thể khóa user này", "Thông báo");
        ...
        private void tl_containmenu_S_FocusedNodeChanged(object sender, DevExpress.XtraTreeList.FocusedNodeChangedEventArgs e)
        {
            try
            {
                ...
                dt=APCoreProcess.APCoreProcess.Read("select mavaitro, status from sysUser where userid='" + Function.clsFunction._iduser + "'");
                if (dt.Rows.Count > 0)
                {
        ...
        private void bbi_vaitro_S_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            frm.TopMost = true;
            frm.passData = new frm_sysRole_S.PassDataUser(GetValueUser);
            frm.them = true;
            frm.dauma = "VT";
            frm.langues = langues;
        ...
        private void loadUser()
        {
            try
            {
                ...
                if (APCoreProcess.APCoreProcess.IssqlCe == false)
                {
                    dt1 = APCoreProcess.APCoreProcess.Read("select distinct sysRole.mavaitro,tenvaitro from sysRole, sysUser where sysUser.mavaitro=sysRole.mavaitro and makethua=@makethua or sysRole.mavaitro=@mavaitro", "sysGrant_Role", new string[2, 2] { { "makethua", getVaiTroByUser(Function.clsFunction._iduser) }, { "mavaitro", getVaiTroByUser(Function.clsFunction._iduser) } }, " (@mavaitro as nvarchar(20), @makethua as nvarchar(20)) ");
                }
                else
                {
                    dt1 = APCoreProcess.APCoreProcess.Read("select distinct sysRole.mavaitro,tenvaitro from sysRole, sysUser where sysUser.mavaitro=sysRole.mavaitro and makethua='" + getVaiTroByUser(Function.clsFunction._iduser) + "' or sysRole.mavaitro='" + getVaiTroByUser(Function.clsFunction._iduser) + "'");
                ...
                for (int j = 0; j < dt1.Rows.Count; j++)
                {
                    ...
                    if (APCoreProcess.APCoreProcess.IssqlCe == false)
                    {
                        dt2 = APCoreProcess.APCoreProcess.Read("select distinct userid as mavaitro,username from sysUser where mavaitro=@mavaitro ", "sysGrant_getUser", new string[1, 2] { { "mavaitro", IDRoot } }, " (@mavaitro as nvarchar(20)) ");
                    }
                    else
                    {
                        dt2 = APCoreProcess.APCoreProcess.Read("select distinct userid as mavaitro,username from sysUser where mavaitro='" + IDRoot + "'");
