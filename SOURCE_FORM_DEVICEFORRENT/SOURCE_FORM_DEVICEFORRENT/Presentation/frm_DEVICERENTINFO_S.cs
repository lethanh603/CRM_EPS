...
{
    public partial class frm_DEVICERENTINFO_S : DevExpress.XtraEditors.XtraForm
    {
        #region Contructor
        public frm_DEVICERENTINFO_S()
        {
            InitializeComponent();
        }
        #endregion
        ...
        private void save()
        {
            ...
            if (_insert == true)
            {
                ...
                Function.clsFunction.sysTrace(this.Name, Function.clsFunction.transLate(btn_insert_allow_insert.Text), txt_iddevice_IK1.Text, txt_statusdevice_500_I2.Text, SystemInformation.ComputerName.ToString(), "0", ds.GetXml(), Assembly.GetAssembly(this.GetType()).ToString().Split(',')[0]);
        ...
        {
            if (txt_devicename_I2.Text == "")
            {
                txt_devicename_I2.Focus();
                dxEp_error_S.SetError(txt_devicename_I2, Function.clsFunction.transLateText("Không được rỗng"));
                return false;
            }
